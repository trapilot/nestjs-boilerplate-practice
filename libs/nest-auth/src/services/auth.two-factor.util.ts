...
export class AuthTwoFactorUtil implements OnModuleInit {
  ...
  constructor(
    ...
    this.step = this.config.get<number>('auth.twoFactor.step')
    this.window = this.config.get<number>('auth.twoFactor.window')
    this.secretLength = this.config.get<number>('auth.twoFactor.secretLength')
    this.challengeTtlInMs = this.config.get<number>('auth.twoFactor.challengeTtlInMs')
    this.cachePrefixKey = this.config.get<string>('auth.twoFactor.cachePrefixKey')
    this.backupCodesCount = this.config.get<number>('auth.twoFactor.backupCodes.count')
    this.backupCodesLength = this.config.get<number>('auth.twoFactor.backupCodes.length')
    this.encryptionKey = this.config.get<string>('auth.twoFactor.encryption.key')
    this.maxAttempt = this.config.get<number>('auth.twoFactor.maxAttempt')
  ...
  generateSecret(): string {
    return this.authenticator.generateSecret(this.secretLength)
  }

  /**
   * Create a key URI for authenticator apps (QR code)
   * @param label User key
   * @param secret TOTP secret
   * @returns Key URI
   */
  createKeyUri(label: string, secret: string): string {
    return this.authenticator.generateURI({
      label,
      secret,
      issuer: this.issuer,
    })
  ...
  verifyCode(secret: string, code: string): boolean {
    return this.authenticator.verifySync({
      token: code,
      secret,
    }).valid
  }
  ...
  generateEncryptionIv(): string {
    ...
    return `hex:${this.helperService.randomBytes(16).toString('hex')}`
  }
  ...
  encryptSecret(secret: string, iv: string): string {
    return this.helperService.aes256Encrypt(secret, {
  ...
  decryptSecret(secret: string, iv: string): string {
    return this.helperService.aes256Decrypt(secret, {
      key: this.encryptionKey,
      iv,
    })
  }
  ...
  generateBackupCodes(): IAuthTwoFactorBackupCodes {
    ...
    return {
      codes,
      hashes: codes.map(code => this.helperService.hashCreate(code, { algorithm: 'sha256' })),
    }
  }
   ...
   */
  verifyBackupCode(backupCodes: string[], input: string): IAuthTwoFactorBackupCodesVerifyResult {
    const codeHash = this.helperService.hashCreate(input, { algorithm: 'sha256' })
    const index = backupCodes.findIndex(hash =>
      this.helperService.hashCompare(hash, codeHash, { algorithm: 'sha256' }),
    )
  ...
  async createChallenge(
    ...
    await this.cache.set<IAuthTwoFactorChallengeCache>(key, cachePayload, this.challengeTtlInMs)
   ...
   */
  async getChallenge(token: string): Promise<IAuthTwoFactorChallengeCache | null> {
    const key = `${this.cachePrefixKey}:${token}`
    const cached = await this.cache.get<IAuthTwoFactorChallengeCache>(key)
  ...
  async verifyTwoFactor(
    twoFactor: IAuthUserTwoFactor,
    { method, code, backupCode }: IAuthTwoFactorVerify,
  ): Promise<IAuthTwoFactorVerifyResult> {
    const normalizedCode =
      method === EnumAuthTwoFactorMethod.code ? code?.trim() : backupCode?.trim()
    ...
    const backupValidation = this.verifyBackupCode(twoFactor.backupCodes, normalizedCode)
   ...
   */
  checkAttempt(user: IAuthUserData): boolean {
   ...
   */
  async lockTwoFactorAttempt(user: IAuthUserData): Promise<void> {
    const key = `${this.cachePrefixKey}:lock:${user.id}`
    const ttlExponentialInMs =
      Math.pow(2, user.twoFactor.attempt / this.maxAttempt) * this.lockAttemptDuration
    await this.cache.set<boolean>(key, true, ttlExponentialInMs)
   ...
   */
  async getLockTwoFactorAttempt(user: IAuthUserData): Promise<number> {
    const key = `${this.cachePrefixKey}:lock:${user.id}`
    const isLocked = await this.cache.get<boolean>(key)
    const retryAfterMs = await this.cache.ttl(key)
