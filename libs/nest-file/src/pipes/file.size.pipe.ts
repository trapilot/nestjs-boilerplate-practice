...
import { EntityValidateBuilder, EntityValidateException, NestAlgorithm } from 'lib/nest-core'
import { FILE_RATIO_MAX_ROUNDING, FILE_SIZE_MAX_ROUNDING } from '../constants'
  ...
  private async validate(file: IFile, metadata: ArgumentMetadata): Promise<void> {
    ...
    for (const size of this.sizes) {
      ...
      if (adjustRatio > FILE_RATIO_MAX_ROUNDING) {
        const commonDivisor = NestAlgorithm.getGCD(width, height)
