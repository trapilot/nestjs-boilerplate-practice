...
export class NestContext {
  private static readonly storage = new AsyncLocalStorage<NestContext>()
  ...
  static create(ctx: NestContext, next: (...args: any[]) => void): void {
    this.storage.run(ctx, next)
  }

  static current(): NestContext | undefined {
