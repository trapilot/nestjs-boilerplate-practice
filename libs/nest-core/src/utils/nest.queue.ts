...
import { NestAlgorithm } from './nest.algorithm'
import { pNestTimeout, TimeoutError } from './nest.timeout'
    ...
    return new Promise<TaskResultType>((resolve, reject) => {
      const run = async (): Promise<void> => {
        ...
        try {
          ...
          const operation =
            this.timeout === undefined && options.timeout === undefined
              ? fn({ signal: options.signal })
              : pNestTimeout(Promise.resolve(fn({ signal: options.signal })), {
                  milliseconds: (options.timeout === undefined ? this.timeout : options.timeout)!,
                  fallback: () => {
                    if (
...
export default class PriorityQueue implements Queue<RunFunction, PriorityQueueOptions> {
  ...
  enqueue(run: RunFunction, options?: Partial<PriorityQueueOptions>): void {
    ...
    const index = NestAlgorithm.lowerBound(
      this.#queue,
      element,
