export class TimeUtil {
  ...
  static format(val: number, options: { long: boolean }) {
    if (isFinite(Number(val))) {
      return options.long ? this.fmtLong(val) : this.fmtShort(val)
    }
    throw new Error('val is not a non-empty string or a valid number. val=' + JSON.stringify(val))
  }
  ...
  private static fmtLong(ms: number): string {
    const msAbs = Math.abs(ms)
    if (msAbs >= 86_400_000) {
      return this.plural(ms, msAbs, 86_400_000, 'day')
    }
    if (msAbs >= 3_600_000) {
      return this.plural(ms, msAbs, 3_600_000, 'hour')
    }
    if (msAbs >= 60_000) {
      return this.plural(ms, msAbs, 60_000, 'minute')
    }
    if (msAbs >= 1_000) {
      return this.plural(ms, msAbs, 1_000, 'second')
    }
    return ms + ' ms'
  }
