...
import { NestContext } from './nest.context'
import { NestTimer } from './nest.timer'

export class NestHelper {
  static sleep(msValue: string | number) {
    return new Promise((r) => setTimeout(r, NestTimer.ms(`${msValue}`)))
  }
  ...
  static toLocaleValue(jsonValue: any, language?: string): string {
    if (jsonValue) {
      return jsonValue[language || NestContext.language()] ?? ''
    }
    return ''
  }
  ...
  static mergeDate(date: Date | string, duration: string): Date {
    ...
    return NestHelper.toDate(reqDate, { duration }) as Date
  }
  ...
  static nowDate(options?: Omit<IDateRequestOptions, 'format'>): Date {
    ...
    return NestHelper.toDate(reqDate, options) as Date
  }
  ...
  static toUrl(path: string, host?: string): string {
    ...
    if (NestHelper.isDevelopment() && NestContext.apiType() === ENUM_APP_API_TYPE.APP) {
