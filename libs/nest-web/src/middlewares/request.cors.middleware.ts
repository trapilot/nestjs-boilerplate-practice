...
export class RequestCorsMiddleware implements NestMiddleware {
  ...
  constructor(private readonly config: ConfigService) {
    this.allowedOrigin = this.config.get<string | boolean | string[]>('request.cors.allowedOrigin')
    this.allowedMethods = this.config.get<string[]>('request.cors.allowedMethods')
    this.exposedHeaders = this.config.get<string[]>('request.cors.allowedHeaders')
    this.exposedHeaders = this.config.get<string[]>('request.cors.exposedHeaders')
  }

  use(req: IRequestApp, res: IResponseApp, next: INextFunction): void {
    const corsOptions: CorsOptions = {
      origin: (origin, callback) => this.originValidator(origin, callback),
   ...
   */
  private matchWildcardOrigin(hostname: string, port: string, pattern: string): boolean {
    const { hostname: patternHostname, port: patternPort } = this.parsePattern(pattern)
