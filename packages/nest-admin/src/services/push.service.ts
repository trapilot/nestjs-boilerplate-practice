...
export const pushService = {
  ...
  mapShorted: async <T = schemas['PushResponseListDto'][]>(
    options?: RequestOptions,
  ): Promise<T> => {
    const query = options?.query || {}
    const search = new URLSearchParams()
    ...
    const qs = search.toString()
    const baseUrl = '/pushes/map-shorted'
    const url = qs ? baseUrl + '?' + qs : baseUrl
    const client = _privateAxios
    const config = options?.config || {}
    const data = await client.get(url, config)
  ...
  get: async <T = schemas['PushResponseDetailDto']>(
    ...
    const qs = search.toString()
    const baseUrl = '/pushes/' + String(params.id)
    const url = qs ? baseUrl + '?' + qs : baseUrl
    const client = _privateAxios
    const config = options?.config || {}
    const data = await client.get(url, config)
  ...
  inactive: async <T = schemas['PushResponseDetailDto']>(
    ...
    const qs = search.toString()
    const baseUrl = '/pushes/' + String(params.id) + '/inactive'
    const url = qs ? baseUrl + '?' + qs : baseUrl
    const client = _privateAxios
    const config = options?.config || {}
    const data = await client.put(url, body, config)
  ...
  active: async <T = schemas['PushResponseDetailDto']>(
    ...
    const qs = search.toString()
    const baseUrl = '/pushes/' + String(params.id) + '/active'
    const url = qs ? baseUrl + '?' + qs : baseUrl
    const client = _privateAxios
    const config = options?.config || {}
    const data = await client.put(url, body, config)
