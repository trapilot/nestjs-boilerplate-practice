...
const qs = search.toString()
  ...
  create: async <T = schemas['MediaResponseDetailDto']>(body?: unknown, options?: RequestOptions): Promise<T> => {
    const query = options?.query || {}
const search = new URLSearchParams()
Object.entries(query).forEach(([k, v]) => {
  if (v === undefined || v === null) return
...
const qs = search.toString()
  ...
  mapShorted: async <T = schemas['MediaResponseListDto'][]>(options?: RequestOptions): Promise<T> => {
    const query = options?.query || {}
const search = new URLSearchParams()
Object.entries(query).forEach(([k, v]) => {
  if (v === undefined || v === null) return
...
const qs = search.toString()
  ...
  get: async <T = schemas['MediaResponseDetailDto']>(params: { id: string | number }, options?: RequestOptions): Promise<T> => {
    const query = options?.query || {}
const search = new URLSearchParams()
Object.entries(query).forEach(([k, v]) => {
  if (v === undefined || v === null) return
...
const qs = search.toString()
  ...
  update: async <T = schemas['MediaResponseDetailDto']>(params: { id: string | number }, body?: schemas['MediaRequestUpdateDto'], options?: RequestOptions): Promise<T> => {
    const query = options?.query || {}
const search = new URLSearchParams()
Object.entries(query).forEach(([k, v]) => {
  if (v === undefined || v === null) return
