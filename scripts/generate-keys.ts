...
class JwtKeysGenerator {
  ...
  constructor(keyDir: string) {
    ...
    this.refreshTokenPrivateKeyPath = path.join(this.keyDir, 'refresh-token.pem')
    this.refreshTokenPublicKeyPath = path.join(this.keyDir, 'refresh-token.pub')
  ...
  createJwks(accessKeyPath: string, refreshKeyPath: string, outputPath: string): void {
    try {
  ...
  generateKeys(): void {
    try {
      ...
      this.generateES512KeyPair(this.accessTokenPrivateKeyPath, this.accessTokenPublicKeyPath)

      // Generate refresh token
      console.log('Generating Refresh Token ES512 keys...')
      this.generateES512KeyPair(this.refreshTokenPrivateKeyPath, this.refreshTokenPublicKeyPath)
...
function main() {
  ...
  const keyDir = argv[1] || join(process.cwd(), 'resources', 'certs', 'keys')
...
Commands:
  generate    Generate JWT ES512 keys
  rollback    Remove JWT ES512 keys

Arguments:
  keysDir     Output directory for keys (default: ./resources/certs/keys)
        `)
  }
}
