...
import { NestTimer } from 'lib/nest-core'
  ...
  (): Record<string, any> => ({
    jwt: {
      accessToken: {
        ...
        expirationTime: NestTimer.seconds(process.env.AUTH_JWT_ACCESS_TOKEN_EXPIRED || '30m'),
      },

      refreshToken: {
        secretKey: process.env.AUTH_JWT_REFRESH_TOKEN_SECRET_KEY ?? `${process.env.APP_NAME}_rs_k`,
        expirationTime: NestTimer.seconds(process.env.AUTH_JWT_REFRESH_TOKEN_EXPIRED || '2h'),
      },
    ...
    password: {
      ...
      maxAttempt: 5,
      saltLength: 8,
      expiredIn: NestTimer.seconds('182d'), // 0.5 years
      expiredInTemporary: NestTimer.seconds('3d'), // 3 days
      period: NestTimer.seconds('90d'), // 3 months
    },
    ...
    otp: { length: 6, maxAttempts: 5, ttl: NestTimer.seconds('5m') },
    token: { length: 32, maxAttempts: 3, ttl: NestTimer.seconds('1d') },
  }),
)
