...
@Injectable()
export class InvoiceProcEarnPointsHandler implements IWorkerHandler {
  topic: string = EnumInvoiceQueue.PROC_EARN_POINTS
  version: number = INVOICE_QUEUE_PROC_VERSION[EnumInvoiceQueue.PROC_EARN_POINTS]
  ...
  async handle(version: number, payload: { ids: number[] }): Promise<void> {
    ...
    for (const invoiceId of payload.ids) {
      await this.invoiceService.processEarnPoints(invoiceId)
    }
  }
}
