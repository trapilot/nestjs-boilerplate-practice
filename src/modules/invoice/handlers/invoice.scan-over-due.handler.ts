...
@Injectable()
export class InvoiceScanOverDueHandler implements IWorkerHandler {
  topic: string = EnumInvoiceQueue.SCAN_OVER_DUE_INVOICES
  version: number = MEMBER_QUEUE_SCAN_VERSION[EnumInvoiceQueue.SCAN_OVER_DUE_INVOICES]
  ...
  async handle(version: number): Promise<void> {
    ...
    await this.scanner.runWithCursor({
      ...
      retrieve: async state => {
        return await this.invoiceService.scanOverDueInvoices(state.lastId, nowDate)
      },
