...
import { ArrUtil, CryptoService, FileService, HelperService, IRequestApp } from 'lib/nest-core'
...
@Injectable()
export class AuthService implements IAuthValidator<TUser> {
  ...
  async createConfirmToken(user: TUser, passwordHash: string): Promise<string> {
    ...
    return ArrUtil.join(
      [
        this.crypto.createHmac(`${user.id}`, { algorithm: 'md5', key: passwordHash }),
  ...
  async verifyConfirmToken(user: TUser, token: string): Promise<boolean> {
    ...
    return (
      this.crypto.compareHmac(`${user.id}`, md5, {
        algorithm: 'md5',
        key: user.passwordConfirm,
      }) &&
      this.crypto.compareHmac(`${timestamp}`, sha256, {
        algorithm: 'sha256',
        key: user.passwordConfirm,
      })
    )
