...
@Injectable()
export class MemberResetExpiryPointHandler implements IQueueHandler {
  topic: string = EnumMemberQueue.RESET_EXPIRY_POINTS
  version: number = 1
  ...
  @OnScope(EnumScopeType.QUEUE, { context: EnumMemberQueue.RESET_EXPIRY_POINTS, async: true })
  async handle(): Promise<void> {
    ...
    const memberPoints = await this.prisma.memberPointHistory.findMany({
      where: {
        isActive: true,
        isDeleted: false,
        expiryDate: { lte: nowDate, not: null },
      },
    ...
    const memberIds = memberPoints.map(i => i.memberId)
    if (!memberIds.length) {
    ...
    // handle job
    await this.memberService.resetMemberPoints(nowDate, memberIds)
