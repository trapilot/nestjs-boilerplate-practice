...
@Injectable()
export class MemberEarnHighestPurchaseHandler implements IQueueHandler {
  topic: string = EnumMemberQueue.EARN_HIGHEST_PURCHASE_IN_BIRTH
  version: number = 1
  ...
  @OnScope(EnumScopeType.QUEUE, {
    context: EnumMemberQueue.EARN_HIGHEST_PURCHASE_IN_BIRTH,
    async: true,
  })
  async handle(): Promise<void> {
    ...
    const extractDate = this.helperService.dateExtract(nowDate)
